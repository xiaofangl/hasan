    ...
    if users:
        ...
        if user:
            ...
            if user.is_active:
                ...
                user_group = get_user_group(user_id=user_id[0].get('id'))
                # print user_group['data']
                user_group_list = []
                if user_group['status']:
                    for i in user_group['data']:
                        user_group_list.append(i['group__name'])

                res = {'status': True, 'msg': '登录成功。。', 'user_id': user.pk, 'userhashid': obj.hashKey, 'code': '0', 'username': username, 'is_admin': is_admin, 'user_group': user_group_list}
            else:
...
def signup(request):
    ...
    try:
        ...
        if user is not None:
            ...
            res['user_group'] = user_group_list
...
def reset_password(request):
    ...
    if first_pwd == second_pwd:
        ...
        if user:
            ...
            obj = userInfo.objects.create(user_id=user_id[0].get('id'), hashKey=hash_key)
            is_admin = run_is_admin(obj.hashKey)
            user_group = get_user_group(user_id[0].get('id'))
            user_group_list = []
            if user_group['status']:
                for i in user_group['data']:
                    user_group_list.append(i['group__name'])
            res = {'status': True, 'msg': '修改成功。。', 'user_id': obj.user_id, 'userhashid': obj.hashKey, 'code': '0', 'username': username, 'is_admin': is_admin, 'user_group': user_group_list}
        else:
