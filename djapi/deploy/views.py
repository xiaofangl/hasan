...
class Oper_ProjectInfo(object):
    ...
    def _list_project(self, name):
        ...
        if name:
            data = ProjectInfo.objects.filter(is_del=False, name=name).values('id', 'name', 'type', 'server_port', 'created', 'describe', 'download_path', 'server_path')
            print 'data', data
            if len(data):
                tmp = []
                for item in data:
            ...
            tmp = []
            data = ProjectInfo.objects.filter(is_del=False).values('id', 'name', 'type', 'server_port', 'created', 'describe')
            for item in data:
                tmp.append(item)
            self.res['data'] = tmp
    ...
    def _create_project(self, name, type, repository, server_port, server_path, download_path, host_list, describe):
        ...
        try:
            data = ProjectInfo.objects.create(name=name, type=type, repository=repository, server_port=server_port, server_path=server_path,\
                                   download_path=download_path, describe=describe)
            self.res['msg'] = 'create success...'
        except IOError as data:
            self.res['msg'] = data
            self.res['status'] = False
        ProjectLog.objects.create(project=name, code=data, oper_type='_create_project', oper_user='', status=self.res['status'])
        # print self.res
    ...
    def _modify_project_list(self, ids):
        print 'ids', ids
        data = ProjectInfo.objects.filter(is_del=False, id=ids).values('id', 'name', 'type', 'server_port', 'created', 'describe', 'repository', 'download_path', 'server_path')
